...
Returns the platform agent card (aggregated skills from all agents). `url` is the
**absolute** base URL of the A2A service (per the A2A spec) — derived from the
incoming request, or pinned via `A2A_PUBLIC_URL` when behind a TLS-terminating proxy:

```json
{
  "name": "OSIR Agent Platform",
  "description": "AI-powered domain registrar...",
  "url": "https://be.osir.com/a2a",
  "version": "1.0.0",
  "documentationUrl": "https://github.com/Osir-Inc/mcp-a2a",
...
> **Discovery path is root-anchored:** `/.well-known/agent.json` — nothing goes in
> front of it (not `/a2a/.well-known/...`). Publicly: `https://be.osir.com/.well-known/agent.json`.
