...
#### VPS Hosting (15)
`listVpsPackages`, `listVpsLocations`, `getVpsPackageDetails`, `orderVps`, `listMyVpsInstances`, `getVpsInstanceDetails`, `deleteVpsInstance`, `changeVpsPaymentTerm`, `loginToVpsPanel`, `countMyVpsInstances`, `listVpsOsTemplates`, `buildVpsInstance`, `addSshKey`, `listMySshKeys`, `deleteSshKey`

> **Ordering and installing an OS are two different VirtFusion steps.** `orderVps` on its own creates a
> server with **no operating system**. To get a server that is ready to use, resolve an
