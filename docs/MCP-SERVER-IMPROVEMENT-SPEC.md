...
| `operatingSystemId` | Integer | No | OS template id to install. Resolve with `listVpsOsTemplates` — ids are per-install and change on re-import. Omit and the server is created with no OS. |
| `sshKeyIds` | List&lt;Integer&gt; | No | Ids of your stored SSH keys to inject during the install |
